...
#FSR 1252, Czernik 37, Collinder 110
...
for i in range(len(cluster_info['Cluster'])):
    ...
    #plt.savefig(cluster_info['Cluster'][i]+'_sky_pos.pdf', bbox_inches='tight', format='pdf')
    #plt.close()
    plt.show()"""
    ...
    #define a function where the machine learning algorithm can input coefficients
    def func(x, a, b, c, d):
        if cluster_info['Cluster'][i] == 'NGC_752' or cluster_info['Cluster'][i] == 'NGC_2682' or cluster_info['Cluster'][i] == 'NGC_1907' or cluster_info['Cluster'][i] == 'Collinder_110' or cluster_info['Cluster'][i] == 'NGC_2660' or cluster_info['Cluster'][i] == 'NGC_2477' or cluster_info['Cluster'][i] == 'NGC_2509' or cluster_info['Cluster'][i] == 'NGC_6939' or cluster_info['Cluster'][i] == 'Melotte_71' or cluster_info['Cluster'][i] == 'Melotte_66' or cluster_info['Cluster'][i] == 'King_5' or cluster_info['Cluster'][i] == 'NGC_1245' or cluster_info['Cluster'][i] == 'NGC_2236' or cluster_info['Cluster'][i] == 'NGC_2420' or cluster_info['Cluster'][i] == 'NGC_2506' or cluster_info['Cluster'][i] == 'NGC_6791':
            return c + a*np.exp(-x*b)
        else:
    ...
    elif cluster_info['Cluster'][i] == 'NGC_6940':
    ...
    plt.plot(np.linspace(0.0,lim*cluster_info['Radius'][i],80), func(np.linspace(0.0,lim*cluster_info['Radius'][i],80), *popt), 'r-')#, label = r'$%5.2f$exp$\bigl\{-(\frac{(x-%5.2f)^2}{2 \cdot %5.3f^2})\bigl\} + %5.1f$' %tuple(popt))
    ...
    plt.title(cluster_info['Cluster'][i].replace('_', ' '))
    plt.legend(frameon=False,fontsize=15)
    #plt.savefig(cluster_info['Cluster'][i]+'_pos_dist.pdf', bbox_inches='tight', format='pdf')
    #plt.close()
    plt.show()"""
    ...
    """if cluster_info['Cluster'][i] == 'NGC_6208':                
        ...
        plt.hist([comp1,comp2,comp3],bins=125,rwidth=1.2,range=(float(cluster_info['Plx'][i])-1,float(cluster_info['Plx'][i])+1),stacked=True,color=[green,purple,darkblue])
    """
    #use scipy.optimize.curve_fit to fit a curve to our data
    ...
    plt.title("Parallax: " + str(cluster_info['Cluster'][i].replace('_', ' ')))
    ...
    plt.yticks(fontsize=15)
    plt.xticks(fontsize=15)
    #plt.savefig(cluster_info['Cluster'][i]+'_plx_dist.pdf', bbox_inches='tight', format='pdf')
    #plt.close()
    plt.show()"""
    ...
    for j in range(len(bin_centers)):
        dplx = bin_centers[j]# - cluster_info['Plx'][i]
    ...
    if cluster_info['Cluster'][i] == 'NGC_6208' or cluster_info['Cluster'][i] == 'NGC_6940' or cluster_info['Cluster'][i] == 'Pismis_18' or cluster_info['Cluster'][i] == 'Collinder_110' or cluster_info['Cluster'][i] == 'Czernik_37' or cluster_info['Cluster'][i] == 'FSR_1252' or cluster_info['Cluster'][i] == 'King_5' or cluster_info['Cluster'][i] == 'NGC_2447':
    ...
    for j in range(len(ra)):
        ...
        if cluster_info['Cluster'][i] == 'NGC_6208'  or cluster_info['Cluster'][i] == 'NGC_6940' or cluster_info['Cluster'][i] == 'Czernik_37' or cluster_info['Cluster'][i] == 'FSR_1252':
            phic = ppmc[j]
            phif = ppmf[j]
        elif  cluster_info['Cluster'][i] == 'NGC_2447' or cluster_info['Cluster'][i] == 'Collinder_110':
            phic = ppmc[j]*pplxc[j]
            phif = ppmf[j]*pplxf[j]
        elif  cluster_info['Cluster'][i] == 'Pismis_18' or cluster_info['Cluster'][i] == 'King_5':
            phic = ppmc[j]*ppos[j]
            phif = ppmf[j]*ppos[j]
        else:
    ...
    hist(total_probs, range=(min(total_probs),max(total_probs)), bins=40, color='#bc66d4')
    title(cluster_info['Cluster'][i].replace('_',' '))
    xlabel('Total prob - old method')
    ylabel('N')
    show()"""
    ...
    #savefig(cluster_info['Cluster'][i] + '_prob_dist.pdf', bbox_inches='tight', format='pdf')
    #close()
    show()
    ...
    ax1.scatter(ra, dec, s=4, c=total_probs, cmap=cm.plasma_r)
    ax1.set_title("RA - dec, old method "+ cluster_info['Cluster'][i].replace('_',' '))
    ax1.set_ylabel(r'Dec (deg)')
    ax1.set_xlabel(r'RA (deg)')
    
    im = ax2.scatter(pmra, pmdec, s=4, c=total_probs, cmap=cm.plasma_r)
    ax2.set_title("pmRA - pmDEC, old method "+ cluster_info['Cluster'][i].replace('_',' '))
    ...
    """# plot RA vs Dec, PMRa vs PMDec, CMDs with probabilities from total_probs2
    fig, (ax1, ax2) = plt.subplots(nrows=1, ncols=2, figsize=(12, 6))
    ax1.scatter(ra, dec, s=4, c=total_probs2, cmap=cm.plasma_r)
    ax1.set_title("RA - dec, new method "+ cluster_info['Cluster'][i].replace('_',' '))
    ax1.set_ylabel(r'Dec (deg)')
    ax1.set_xlabel(r'RA (deg)')
    
    im = ax2.scatter(pmra, pmdec, s=4, c=total_probs2, cmap=cm.plasma_r)
    ax2.set_title("pmRA - pmDEC, new method "+ cluster_info['Cluster'][i].replace('_',' '))
    ax2.set_ylabel(r'pmdec (mas yr$^{-1}$)')
    ax2.set_xlabel(r'pmRA (mas yr$^{-1}$)')
    divider = make_axes_locatable(ax2)
    cax = divider.append_axes("right", size="5%", pad=0.05)
    ...
    xlim([-2.0,6.0])
    ylim([22.0,6.0])   
    title('Color Magnitude Diagram: '+ cluster_info['Cluster'][i].replace('_',' '))
    xlabel('BP-RP')
    ylabel('G (mag)')
    ...
    """#make new csv files to feed into panstarrs
    ...
    cross_ref_ra = []
    cross_ref_dec = []
    if cluster_info['Cluster'][i] in cluster_names:
        for j in range(len(total_probs2)):
            if cluster_info['Cluster'][i] == 'NGC_6791':
                if total_probs2[j] >= 0.2 and g[j] <= 22:
                    cross_ref_target.append(ID[j])
                    cross_ref_ra.append(ra[j])
                    cross_ref_dec.append(dec[j])
            else:
                if total_probs2[j] >= 0.2 and g[j] <= 16.0:
                    cross_ref_target.append(ID[j])
                    cross_ref_ra.append(ra[j])
                    cross_ref_dec.append(dec[j])
        cross_ref_file = Table()
        cross_ref_file['target'] = np.array(cross_ref_target)
        cross_ref_file['ra'] = np.array(cross_ref_ra, dtype = float)
        cross_ref_file['dec'] = np.array(cross_ref_dec, dtype = float)
        cross_ref_file.write('/Users/abbychriss/Desktop/'+cluster_info['Cluster'][i]+'_cross_ref.csv', overwrite=True)"""
    ...
    ascii.write(cluster_out2, cluster_info['Cluster'][i]+'_out2.dat', overwrite=True)"""
...
for i in range(len(cluster_info['Cluster'])):
    if cluster_info['Cluster'][i] in cluster_keep:
        print(cluster_info['Cluster'][i],round(opening_angles[i],5))
