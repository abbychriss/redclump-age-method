...
print("\n",table1_2020['Cluster','AgeNN','plx'][np.where(table1_2020['Cluster']=='NGC_7789')[0]])
...
for row in range(len(age_f)):
    if 8.3 <= age_f[row] <= 9.3:
        ...
        cluster_indexes_t.append(row)
        
#make new lists for certain parameters for filtered clusters      
new_clusters_t = [cluster_names[i] for i in cluster_indexes_t]
new_nbstars07_t = [np.array(table1_2020['nbstars07'])[i] for i in cluster_indexes_t]
new_RAdeg_t = [np.array(table1_2020['RAdeg'])[i] for i in cluster_indexes_t]
new_DEdeg_t = [np.array(table1_2020['DEdeg'])[i] for i in cluster_indexes_t]
new_pmRA_t = [np.array(table1_2020['pmRA*'])[i] for i in cluster_indexes_t]
new_pmDE_t = [np.array(table1_2020['pmDE'])[i] for i in cluster_indexes_t]
new_plx_t = [np.array(table1_2020['plx'])[i] for i in cluster_indexes_t]
...
#filter clusters so that plx > 0.5 AND 8.3 <= AgeNN <= 9.3

cluster_indexes_plx_t = []
for i in range(len(table1_2020['Cluster'])):
    if 8.3 <= age_f[i] <= 9.3 and plx_f[i] > 0.5:
        cluster_indexes_plx_t.append(i)

#make new lists for certain parameters for clusters filtered with respect to plx > 0.5 (equiv, dist < 2 kpc) and age (8.3 <= log t <= 9.3)
new_ages_plx_t = [age_f[i] for i in cluster_indexes_plx_t]
new_clusters_plx_t = [cluster_names[i] for i in cluster_indexes_plx_t]
new_nbstars07_plx_t = [np.array(table1_2020['nbstars07'])[i] for i in cluster_indexes_plx_t]
new_RAdeg_plx_t = [np.array(table1_2020['RAdeg'])[i] for i in cluster_indexes_plx_t]
new_DEdeg_plx_t = [np.array(table1_2020['DEdeg'])[i] for i in cluster_indexes_plx_t]
new_pmRA_plx_t = [np.array(table1_2020['pmRA*'])[i] for i in cluster_indexes_plx_t]
new_pmDE_plx_t = [np.array(table1_2020['pmDE'])[i] for i in cluster_indexes_plx_t]
new_plx_plx_t = [np.array(table1_2020['plx'])[i] for i in cluster_indexes_plx_t]

#Write ASCII table for clusters with average distance < 2kpc and age (8.3 <= log t <= 9.3) stored in file called 'clusterplx_ages.dat'
filtered_clusters_plx_t = Table()
filtered_clusters_plx_t['Cluster'] = np.array(new_clusters_plx_t, dtype=str)
filtered_clusters_plx_t['AgeNN'] = np.array(new_ages_plx_t, dtype=float)
filtered_clusters_plx_t['nbstars07'] = np.array(new_nbstars07_plx_t)
filtered_clusters_plx_t['RAdeg'] = np.array(new_RAdeg_plx_t)
filtered_clusters_plx_t['DEdeg'] = np.array(new_DEdeg_plx_t)
filtered_clusters_plx_t['pmRA*'] = np.array(new_pmRA_plx_t)
filtered_clusters_plx_t['pmDE'] = np.array(new_pmDE_plx_t)
filtered_clusters_plx_t['plx'] = np.array(new_plx_plx_t)
ascii.write(filtered_clusters_plx_t, 'clusterplx_ages.dat', overwrite=True)

filtered_clusters_plx_t = Table.read('/Users/abbychriss/Desktop/WSU/clusterplx_ages.dat', format='ascii')

print(filtered_clusters_plx_t['Cluster','AgeNN','plx'][np.where(filtered_clusters_plx_t['Cluster']=='Melotte_111')[0]])
